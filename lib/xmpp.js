...
function XMPPClient(jid){
    ...
    this.xmppParser = {
        ...
        presence: function(sStatus, sShow, attr){
            ...
            var zustand = 'available'; // TODO use 'type' above.
            //if(sShow) zustand = 'available';// TODO detailed status like away, chat, dnd, ...
            if('unavailable' == type) zustand = 'unavailable';

            var showstr = '';
            if(sShow) showstr = sShow.children.join('');
        ...
        setStatus: function(jid, status, statusText){
            try{
                var r = self.rosterManager._roster,
                    jid = new $.node.xmpp.JID(jid).bare().toString();
                for(var i in r){
                    if(!r[i][jid]) continue;
